...
However, it is important to distinguish OOTA cycles from the simple
reordering exemplified by
Listing~\ref{lst:Simple Reordering}~\cite{PaulEMcKenney2020RelaxedGuideRelaxed}.
Both the C++ implementation\footnote{
	This paper will use ``C++ implementation'' both excluding the
	CPU, as here, and including the CPU.
	Explicit disambiguation is added when necessary.}
and the CPU are within their rights to reorder lines~12 and~13, which
can result in all of \co{x}, \co{y}, \co{r1}, and \co{r2} having the
value 42 as follows:
...
that there is nothing in the code or in the initial values that would
...
standard that excluded this prohibition, but was otherwise identical
to full C++.
...
of concurrent code fragments, and further has some understanding of OOTA.
...
shows a code fragment that in strict C++ would have an OOTA cycle.
...
it, and this \emph{semantic dependency} results in compilers emitting
instructions such that the hardware enforcement applies.\footnote{
	The need to prohibit simple OOTA is one reason why compiler-based
	value speculation optimizations are frowned upon.}
Roughly speaking, there is semantic dependency from a given load to
a given store when a change in the value loaded that can change the
value stored.

\subsection{OOTA: rf vs. rfe}
\label{sec:OOTA: rf vs. rfe}

However, the ordering implied by the semantic dependency is only one
type of link in an OOTA cycle.
...
standard, except for code fragments exemplified by
Listing~\ref{lst:OOTA With an Intra-Process Store-Load Link}.
This is an elaboration of
Listing~\ref{lst:Simple OOTA}
that adds \co{z} and adds lines~15 and~16 to \co{thread2()}.
The problem is that the C++ implementation is within its rights to
note that line~16 might well execute immediately after line~15, so that
no other thread had a chance to modify \co{z}.
Such an implementation might therefore behave as if the source code
had instead been as shown in
Listing~\ref{lst:OOTA With an Intra-Process Store-Load Link, Optimized},
with line~16 optimized away in favor of line~17 storing the value
in \co{r2} that was loaded from \co{y}.
And given that the load from \co{z} no longer exists, it cannot possibly
act as a temporal constraint.

In order to avoid these backwards-in-time rfi links, we substitute rfe
...
Therefore, the code above has a a semantic dependency when run on systems
choosing signed \co{char}, but not when run on systems choosing unsigned
\co{char}.
...
affected data was being used by only one process, and might react by
