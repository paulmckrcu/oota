...
in a sense to be discussed in
section~\ref{sec:Volatile and Partially Volatile Accesses} below.
...
Such a transformation complies with the strict C++ standard,
...
threads, and therefore it knows that it will not violate the strict C++
...
``partially volatile'', in that the compiler is allowed to omit,
merge, or reorder them but not invent them.

Just what does this mean?

\subsection{Volatile and ``Partially Volatile'' Accesses}
\label{sec:Volatile and Partially Volatile Accesses}
...
To express this idea in more formal terms, we will augment the
...
\item	The map is total: Every access in $A$ to a volatile object
	...
	volatile objects.)
\end{itemize}
Most of these are direct consequences of the fact that volatile
accesses are considered to be a form of I/O.

For accesses to partially volatile objects, we remove some of the
requirements and add a new one.
We leave out the last two items; compilers are allowed to merge or
omit accesses to partially volatile objects.
And the requirement for order preservation is weakened; it applies
only to pairs of accesses to the same object.
Accesses to different objects may be reordered relative to each other.
The new requirement is that the map must preserve the rf relation.
That is, if load $R$ in $A$ reads from store $W$ and $R$ is not
omitted then the instruction it maps to in $H$ must read from the
instruction that $W$ maps to,
and consequently $W$ must not be omitted either.
...
\item	An \co{add} instruction would typically have two inputs
	...
	flag bits---e.g., Zero, Carry, and Overflow---to be stored in
	a status register).
\item	A conditional branch instruction would have only one input:
	the flag bits it tests in the status register.
	A computed branch instruction would as input the register
	containing the address to branch to.
	For both, the output is the new address to be written to the
...
\end{itemize}
Of course, some of an instruction's inputs may be immediate constants
taken from the instruction itself rather than outputs from some
earlier instruction.
Note: Hardware dependency analysis considers only the information
flowing \emph{within} a CPU, not the information flowing between the
CPU and memory, which is handled separately.
...
instruction or a thread's initial state (or possibly some sort of
input instruction, but we will not consider that here).
...
instruction with some well-defined collection of inputs and outputs
or else to abandon it.
Thanks to the law of cause and~effect, a CPU is not able to commit an
instruction until all of its inputs' sources have committed.
(Or at least, all that can affect the action and outputs of the
instruction.
For instance, although no existing architectures do this, there is nothing
in principle to prevent a CPU from committing a \co{multiply}
instruction as soon as either one of its inputs has committed to the
value zero.)
The reason is obvious: An input is subject to change at any time until
its source commits, the outputs aren't determined until the inputs
are, and the instruction can't commit until its outputs are fully
determined.

This means that, with a few possible exceptions such as the
multiply-by-zero example, an instruction can't commit until all the
instructions it depends on have committed.
And even taking the exceptions into account, if a change in the
outputs of instruction $I$ would lead to a change in the inputs to
instruction $J$ then $I$ must commit before $J$.
...
The simplest of these is a load reading from a store in another
...
load until the store commits or later.
...
until then the CPU cannot know whether the instruction should
be executed at all.
Therefore instructions following a conditional or computed branch
instruction in an execution must commit after anything the branch
depends on.
(Again, there could theoretically be an exception for a branch that
conditionally jumps to the immediately following instruction.
We can ignore this issue by treating such branches as no-ops.)

Yet another mechanism for ordering crops up when a load reads from a
store in the same thread (the rfi relation).
In this situation the load cannot commit before the sources for the
store's inputs.
Until the source for the store's value commits, the value of the load
isn't fully determined and so the load cannot commit.
And in general, until the source for store's address commits, it's not
fully determined whether the store and load are for the same address.
This argument doesn't hold if the store and load have the \emph{same}
source for their addresses, but another factor is also at play.
Until the address is fully determined, there is a possibility that it
will turn out to be invalid (a NULL pointer, for example), causing the
store instruction to encounter an addressing exception.
If this should happen the load might not execute at all, and therefore
it can't commit until the address's source has committed.
...
which all atomic objects are treated as partially volatile.
\begin{quote}
...
semantic dependencies satisfy the Fundamental Property, no OOTA
abstract execution can be realized.

Indeed, suppose that abstract execution $A$ is realized by hardware
execution $H$ and $A$ has an OOTA cycle.
This means there are stores $W_i$ semantically depending on loads
$\{R_{i,j}\}$ where each of the loads reads from one of the $W_i$
stores in a different thread.
Let $W_i'$ and $R_{i,j}'$ be the hardware instructions these accesses
map to, if they aren't omitted.
Assuming that at least one of the stores is not omitted,
one of the $W_i'$, call it $W_{i_0}'$, must commit first.
By the Fundamental Property, one of the loads that $W_{i_0}'$ depends on,
call it $R_{i_0,j_0}'$, is not omitted and is ordered before $W_{i_0}'$.
But now we have a contradiction:
\begin{itemize}
\item	$W_{i_0}'$ commits after $R_{i_0,j_0}'$;
\item	$R_{i_0,j_0}'$ commits after the store $W_k'$ it reads from;
\item	$W_k'$ commits no earlier than $W_{i_0}'$.
\end{itemize}

If all the stores in the OOTA cycle are omitted then all the reads
must be omitted as well.
This amounts to a situation where the entire cycle has been optimized
