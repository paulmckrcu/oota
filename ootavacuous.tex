...
bear additional constraints from from hardware architecture and
design,\footnote{
...
must eventually be committed, but that nothing can be committed
until all of the computations that determine those effects have
committed.
...
execute only after the committing of all loads whose return values are
used to compute that store's address or value.
...
However, even duplicating an existing atomic load can lead to incorrect
results.
...
Therefore, C++ implementations should avoid inventing atomic loads,
and in fact should avoid even duplicating them.
...
nonnegative can also prove that there is no semantic dependency between
...
otherwise the preserved nonsemantic dependency prevents an
...
Less-omniscient implementations will preserve the nonsemantic
...
A less-capable implementation would leave the nonsemantic dependency
...
The sum of \co{r2} and \co{r3} is nonzero, justifying line~21's
store to \co{x}.
...
The sum of \co{r2} and \co{r3} is nonzero, justifying line~27's
store to \co{u}.
