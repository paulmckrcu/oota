...
store.\footnote{
	...
	OOTA cycle, for example, by retaining the loads from \co{x}
	and \co{y}.}
...
And then so must $W'$, because the execution of any instruction following
a conditional- or computed-branch instruction is ordered after that
branch.
QED.
...
Future work might include the effects of input and output, as well
as code that reads out the current time, that uses thread IDs,
or that operates on pointer values.

Finally, future work might more precisely delineate the limits of
quasi-volatile behavior.
