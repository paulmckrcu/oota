...
\co{x} was ten, then the value of \co{r1} will be twenty, which is not
a perfect square.
...
\end{verbatim}
\end{quote}
Again, a developer might be forgiven for assuming that the value of
\co{r1} is guaranteed to be a perfect square, at least for sufficiently
...
in which the developer expected line~19 to pass two fields from the
same structure to \co{do_something_with()}, which is an entirely
reasonable expectation.
This expectation will not always be met if the compiler is permitted
to invent atomic loads as shown by the commented-out line~15.
\begin{listing}[tbp]
\scriptsize
\begin{verbatim}
 1 struct foo {
...
Then, assuming that \co{do_something()} is synchronization-free,
the second load on line~15 could be removed without violating user
expectations.
...
their values, but compilers are permitted to merge adjacent atomic loads from the
same variable.
