...
In cases where this is undesirable, the user can resort to volatile
atomic stores or to inline assembly\footnote{
...
code cannot create an OOTA cycle.
After all, the implementation is already within its as-if rights to
assume that the first of a back-to-back pair of non-volatile relaxed
atomic stores was omitted from the source code.
...
is to be allowed.
However, the herd7 tool does not find this litmus test's OOTA cycle.
...
is to be allowed.
The herd7 tool finds this OOTA cycle.
...
12 }
13
14 P1(volatile atomic_int *x, volatile atomic_int *y) {
...
17 }
18
19 P2(volatile atomic_int *x, volatile atomic_int *y) {
...
is to be forbidden.
The herd7 tool finds this OOTA cycle, but flags it as such using \co{S08}.
...
\begin{verbatim}
 ...
 7
 8 P0(atomic_int *x, atomic_int *y, atomic_int *z) {
 9   int r0 = atomic_load_explicit(y, memory_order_relaxed);
10   int r1 = atomic_load_explicit(z, memory_order_relaxed);
11   int r2 = r0 + r1;
12   atomic_store_explicit(x, r2, memory_order_relaxed);
13 }
14
15 P1(atomic_int *x, atomic_int *y, atomic_int *z) {
16   int r3 = atomic_load_explicit(x, memory_order_relaxed);
17   atomic_store_explicit(y, r3 / 2, memory_order_relaxed);
18 }
19
20 P2(atomic_int *x, atomic_int *y, atomic_int *z) {
21   int r4 = atomic_load_explicit(x, memory_order_relaxed);
22   atomic_store_explicit(z, r4 / 2, memory_order_relaxed);
23 }
24
25 locations [0:r0;0:r1;0:r2;1:r3;2:r4]
26 exists(x=42 /\ y=21 /\ z=21)
\end{verbatim}
