...
\begin{document}
\title{P3064R0: How to Avoid OOTA Without Really Trying}
...
Here the load is not actually invented, but instead fused with an
earlier load with no synchronization between the two loads.
...
\begin{quote}
\begin{verbatim}
int r0 = x.load(memory_order_relaxed);
int r1 = r0 * r0 + 2 * r0 + 1;
\end{verbatim}
\end{quote}
A developer might be forgiven for assuming that the value of \co{r1}
is guaranteed to be a perfect square, at least for sufficiently small
values of \co{x}.

But if the compiler is permitted to invent atomic loads, there is no such
guarantee, as can be seen from this transformed version of the above code:
\begin{quote}
\begin{verbatim}
int r0 = x.load(memory_order_relaxed);
int invented = x.load(memory_order_relaxed);
int r1 = r0 * r0 + 2 * invented + 1;
\end{verbatim}
\end{quote}
For example, if the load into \co{r0} happened when the value of \co{x}
was zero and the load into \co{invented} happened when the value of \co{x}
was ten, then the value of \co{r1} will be twenty, which is a perfectly
legitimate integer, but not a perfect square.
...
\begin{quote}
\begin{verbatim}
do_something(x.load(memory_order_relaxed));
int r0 = x.load(memory_order_relaxed);
int r1 = r0 * r0 + 2 * r0 + 1;
\end{verbatim}
\end{quote}
This again ensures that \co{r1} will be a perfect square given sufficiently
small values of \co{x}.
But is the compiler allowed to transform this example into the following?
\begin{quote}
\begin{verbatim}
repurposed = x.load(memory_order_relaxed);
do_something(repurposed);
int r0 = x.load(memory_order_relaxed);
int r1 = r0 * r0 + 2 * repurposed + 1;
\end{verbatim}
\end{quote}
Again, this can result in the value of \co{r1} being something other than
a perfect square.
%
But it would be just fine if the compiler fully fused the two loads from
...
For example, consider a load from an atomic pointer and subsequent uses
of the pointed-to fields:
\begin{quote}
\begin{verbatim}
do_something(gp.load(memory_order_relaxed));
p = gp.load(memory_order_relaxed);
do_something_else(p->a, p->b,p->c);
\end{verbatim}
\end{quote}
Users are within their rights to assume that the three arguments to
\co{do_something_else()} will be taken from the same instance of the pointed-to
structure, at least assuming that the lifetime of that structure extends
across all of the above statements.
However, further assuming that \co{do_something()} is free of
synchronization, a compiler would be within its rights to fuse the two
loads from \co{gp}, for example, as follows:
\begin{quote}
\begin{verbatim}
p = gp.load(memory_order_relaxed);
do_something(p);
...
It is imperative that users be able to rely on arithmetic and pointer accesses.
Therefore, compilers must refrain from inventing atomic loads and from repurposing
their values, but compilers are permitted to fuse adjacent atomic loads from the
same variable.
