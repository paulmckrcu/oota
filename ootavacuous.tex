...
\begin{document}
\title{P3064R3: How to Avoid OOTA Without Really Trying (Informational)}
...
\date{November 15, 2025 (Post-Kona)}
\maketitle{}
...
\begin{abstract}
	...
	{ \bf Note}: Much of this paper has been superseded by P3692,
	so that this paper serves primarily as background material and
	as a repository for litmus tests.
	This means that the most important parts of this paper are
	its appendices.
\end{abstract}
...
We begin with a summary of what needs to change and a brief overview of
the OOTA problem, followed by an equally brief summary of prior work in
this area, and ending with a quick overview of the \co{herd7} tool that
will be used to evaluate litmus tests.

\subsection{What Needs to Change?}
\label{sec:What Needs to Change?}

What changes are needed to prevent OOTA from C++ implementations using
traditional compilers and traditional hardware, including CPUs and
GPGPUs?

\begin{itemize}
\item	To the standard:
	No changes.
	The indefinite prohibition of out-of-thin-air values needs
	to remain.
\item	To implementations:
	No changes.
	Future implementations must avoid optimizations that invent,
	duplicate, or repurpose atomic loads.
	However, such optimizations must be avoided for other reasons,
	including preserving basic laws of arithmetic
	(see Appendix~\ref{app:Inventing Atomic Loads} for more detail).
\item	To user code:
	No changes.
	Users must avoid undefined behavior in portable code, just as
	is the case now.
\end{itemize}

In short, no changes are needed because current implementations already
avoid OOTA cycles.
The point of this paper is to give a semi-formal demonstration of
...
\begin{listing}[tbp]
@@ DisplayRunLitmus litmus/rfub.litmus @@
\caption{Read From Untaken Branch (RFUB)}
\label{lst:Read From Untaken Branch (RFUB)}
\end{listing}

Listing~\ref{lst:Read From Untaken Branch (RFUB)}~\cite{HansBoehm2019OOTArevisitedAgain}
shows a litmus test that demonstrates how compilers' single-threaded
value-range analysis can result in optimizations and reorderings that
give OOTA-like results.
The compiler could notice that by the time execution reached line~19,
the value of \co{r2} was always 42, courtesy of value-range analysis
confined to \co{P1()}.
This would permit the compiler to substitute 42 for \co{r2} in line~19,
which would leave the compiler free to reorder this store to precede
the load on line~13.
This would permit the lines~19, 8, 9, and~13 to execute in that order,
so that both \co{r1} and \co{r2} were assigned the value 42.
This would in turn cause the \co{if} condition on line~15 to always
evaluate to \co{false}, so that line~16 never executed, thus leaving
\co{r3} with its initial value of zero, thus satisfying the
counterintuitive \co{exists} clause on line~23.
All of these optimizations and reorderings are commonplace and thus
assumed valid, but the outcome makes it appear that the final values
of 42 for \co{r1}, \co{r2}, \co{x}, and \co{y} originated at line~17,
which is in a branch of the \co{if} statement that was not taken
in the counter-intuitive execution!

It is generally agreed in the C++ Standards Committee's Study Group 1
(SG1, Concurrency) that RFUB is not an OOTA outcome.
And as can be seen in the figure, \co{herd7} does not give this outcome,
and this is due to the fact that it does not model compiler optimizations.

\begin{listing}[tbp]
@@ DisplayRunLitmus litmus/oota-data+lt100.litmus @@
\caption{NOOTABOGA Value-Range Analysis Destroys sdep}
\label{lst:NOOTABOGA Value-Range Analysis Destroys sdep}
\end{listing}

Another class of non-trivial semantic dependencies result in near-OOTA
based on global analysis (NOOTABOGA), for example, as shown
in Listing~\ref{lst:NOOTABOGA Value-Range Analysis Destroys sdep},
which sprang from the fertile imagination of Hans Boehm.
Value-range analysis would conclude that only the values 0 and 42 are
ever stored to \co{x}, and thus the \co{if} statement on line~14 will
only ever execute its \co{then} clause on line 15.
This conclusion would permit the compiler to replace lines~14-17 with
just line~15.
Both the compiler and the CPU would then be permitted to reorder the store
to \co{x} before the load from \co{y}, thus giving the OOTA-like result
shown on line~21.

\begin{listing}[tbp]
@@ DisplayRunLitmus litmus/oota-data+lt100+wrlx.litmus @@
\caption{NOOTABOGA Value-Range Analysis Disabled}
\label{lst:NOOTABOGA Value-Range Analysis Disabled}
\end{listing}

As noted by Alan Stern, this value-range analysis can be defeated by
adding a store to \co{y} that is greater than 100, as shown in
Listing~\ref{lst:NOOTABOGA Value-Range Analysis Disabled}.
This store could of course happen just before \co{P1()}'s store to \co{y},
which means that \co{herd7} shows a superset of the outcomes for
Listing~\ref{lst:NOOTABOGA Value-Range Analysis Destroys sdep}.

There is some disagreement as to whether the results shown in
Listings~\ref{lst:NOOTABOGA Value-Range Analysis Destroys sdep}
and~\ref{lst:NOOTABOGA Value-Range Analysis Disabled}
are in fact OOTA.
However, the fact that global analysis is required to determine whether
or not this result is permitted makes this result considerably less
friendly to the developer than the RFUB litmus test shown in
Listing~\ref{lst:Read From Untaken Branch (RFUB)}.
This motivates excluding these NOOTABOGA outcomes, perhaps by requiring
compilers to treat non-omitted non-volatile atomic loads as though they
were volatile, at least with respect to the handling of the values that
they return.
A more direct and precisely targeted approach is to forbid applying
value-range analysis to values returned from atomic loads.
...
% Done as of December 18, 2023 %% as of March 18, 2024
%% David Goldblatt
...
P3064R3 adds RFUB and NOOTABOGA litmus tests, notes its informational
status (and the primacy of its appendices), and points at P3692 as
its non-informational counterpart.

P3064R2 incorporates feedback from the St.~Louis meeting, primarily
incorporating clear statements that nothing need change to avoid OOTA.
In other words, the title really means what it says.

P3064R1 adds non-OOTA examples that show that inventing, duplicating,
or repurposing atomic loads can violate basic laws of arithmetic.
