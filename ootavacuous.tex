...
in \co{r2} that was stored to \co{y} by line~2.
...
The same is theoretically true for the nondeterministic aspects of an
execution.
...
\end{verbatim}
\end{quote}
Because the order of evaluation of the division's arguments is undetermined,
...
in unsequenced expressions constitute undefined behavior.
However there are proposals to make them defined in both
C++ and C,\footnote{
...
which would render this theoretical possibility quite real.)
...
anything else.
We will take it to imply that a compiler may not
invent, omit, or merge volatile accesses, in a sense to be discussed below.
