...
\begin{document}
\title{D3064R0: Is OOTA Execution Vacuous for Real-World C++ Implementations?}
...
\begin{abstract}
	...
	These constraints prevent OOTA cycles containing only volatile
	atomics from occurring in UB-free executions of C++ programs
	running on correctly constructed C++ single-thread analysis
	implementations in our current universe.
	Given current optimizations, these constraints also prevent
	OOTA cycles involving non-volatile atomics.
\end{abstract}
...
act for the C++ implemenation to combine loads or stores to adjacent
...
OOTA cycles from volatile atomics when running on real systems in this
universe.
It has also presented a substantial list of optimizations that avoid
creation of OOTA cycles from non-volatile atomics.
...
application of temporal reasoning to OOTA analysis of the C++ memory
model.
...
cycles in UB-free executions of programs using volatile atomics.
...
OOTA cycles in UB-free executions of programs using non-volatile as well
as volatile atomics.
