...
Maged Michael\\{\small \email{maged.michael@gmail.com}} \and
...
also defines quasi-volatile properties for nonvolatile atomic operations.
...
or computed-branch instruction is ordered after that branch.
QED.
...
bear additional constraints from hardware architecture and
design,\footnote{
	Which is in turn constrained by the laws of physics.}
which has been only partially accounted for in the standard and in
prior work.
...
To see this, keep in mind that on real systems, observable effects
must eventually be committed, but that no effect can be committed
while any of the computations that determine the effect remain
speculative.
Furthermore, instructions take time to execute, even when executed
speculatively.
...
Then line~11 can cause the value 42 to be loaded into \co{r2},
thus satisfying the \co{exists} clause on line~20, when
...
Therefore, a given load is prohibited from committing until after the
store that it reads from has committed.
...
As a special case of this prohibition, if a speculated load obtains
a value from a store that has not yet committed, then if that store
is squashed, the load must also be squashed, even if that load and
that store are executed by different CPUs.

Applying this to Listing~\ref{lst:OOTA Cycle}, lines~10 and~12 cannot be
committed until after the load in line~8 and the conditional on line~9
have both committed.
Similarly, lines~18 and~20 cannot be committed until after lines~16
and~17 have committed.

One could imagine a conditional-store instruction, similar to existing
conditional-move instructions, but with condition controlling a store
to memory rather than a move to a register.
The condition would control whether or not the store was executed,
and prior conditional-move instructions could control the value
stored and the address stored to.
With or without prior conditional-move instructions, but assuming a
compiler doing single-thread analysis, a conditional-store instruction
permits generation of code corresponding to lines~10 and~11 of
Listing~\ref{lst:Causality Test Case 2}
on page~\pageref{lst:Causality Test Case 2}
without use of a conditional branch.
This means that the control dependency would extend from the loads on
lines~8 and~9 to the store on line~11 and nowhere else, potentially
enabling more hardware optimizations than would be permitted for code
relying on conditional branches.
Nevertheless, the conditional store could not be committed until all of
the instructions it depends on have committed.
...
all loads in that set must have committed, which in turn requires that
all store providing the loads' values must also have committed.

The key point is that a store having a semantic dependency on a given
set of loads must execute after the completion of at least some of those
...
machine.
This means that any volatile atomic operation involving a store may
execute only after after all loads whose values are used to compute the
store's address or value have committed.
...
significant constrants over those related to nonvolatile non-atomic
stores.
...
Section~\ref{sec:Volatile and Quasi Volatile Accesses}
prohibits reordering only of quasi volatile accesses to the same object,
but such reordering is in any case directly forbidden by the standard.
Referring also to the ``onto'' bullet from
Section~\ref{sec:Volatile and Quasi Volatile Accesses},
implementations should not behave as if the source code has
additional:

\begin{enumerate}
\item	Invented atomic stores.
\item	Duplicated atomic stores.
\item	Invented atomic loads.
\end{enumerate}
...
nonvolatile atomic loads from adjacent objects.
These topics correspond to the ``one-to-one'' and ``total'' bullets of
Section~\ref{sec:Volatile and Quasi Volatile Accesses}.
...
excessive cache-miss overheads.
Even duplicating an atomic load can lead to incorrect results if done
carelessly.
To see this, please refer to
Listing~\ref{lst:Do Not Duplicate Atomic Loads}.
...
\co{foo} structure, something that \co{do_something_with()} might
...
18 }
\end{verbatim}
\caption{Do Not Duplicate Atomic Loads}
\label{lst:Do Not Duplicate Atomic Loads}
\end{listing}

For these reasons, C++ implementations should avoid inventing atomic
loads, and in fact should avoid even duplicating them.

\paragraph{Omitted Redundant Atomic Stores}
In contrast, a pair of back-to-back nonvolatile atomic stores to \co{x}
...
Suppose there are a pair of adjacent nonvolatile atomic objects,
...
This provides an example of the ``one-to-one'' bullet of
Section~\ref{sec:Volatile and Quasi Volatile Accesses}.
...
OOTA cycles in UB-free programs using nonvolatile as well
as volatile atomics.
...
that in this execution the variables \co{r1} and~\co{r2} were never
...
occur in this execution because \co{r3} is zero.
...
In this execution, \co{P0()} has a semantic dependency from line~9 to
...
This execution has an OOTA cycle involving \co{u} and~\co{v}.
...
\section{Additional Litmus Tests}
...
This appendix evaluates them using the \co{herd7} tool and also using
manual analysis.
...
But if \co{x} is a nonvolatile atomic, then there are no worries about
data races.
...
load from \co{y} returned nonzero, which similarly emphasizes the lack
...
that \co{w} becomes nonzero, a change in \co{x} further changes
...
Because both \co{x} and \co{y} are nonvolatile, the compiler is
...
store to the beginning of the nonwinning store that started latest.
...
However, Figure~\ref{fig:On x86, coe Links Form a Coherent Partial Order}
...
consistent with a number of global orders, as required.\footnote{
	Note that this data was not measured on the 80-CPU system, but
	rather on a 16-CPU x86 laptop to avoid an unreadable diagram
	containing 79 bubbles.}
Working from the far left, one CPU saw the values 0, 11, and 9,
a second CPU saw 3, 11, and 9, and so on.
Although different CPUs saw different sequences of values, there is no
disagreement on the order of the values that they could see.
For example, all CPUs agree that the value 9 came last.
This is an example of single-variable sequential consistency, and
...
CPU~0 stores 1 to \co{x} after CPU~3 stores 2 to \co{x}, but
the ordering of the stores is decided after the fact.
...
One way that this could happen is that the cache line containing
\co{x} happened to arrive at CPU~3 before arriving at CPU~0.
A more fanciful explanation is that both values arrived at the
interconnect at the saem time, and the interconnect made an
...
Because CPU~3's load cannot obtain the value stored by CPU~0 until
...
In other words, user preference can exert a nontrivial influence over
