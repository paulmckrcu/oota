...
can be also recast as a \co{herd7} litmus test, as shown in
...
physics would expect.\footnote{
	...
	some of those whose whose work has never strayed too close to
	hardware.}
...
which demonstrates that intra-thread reads-from (rfi) links may be
...
described by the ``exists'' clause.
...
where the store to \co{z} has been moved up to line~9.\footnote{
	...
	not used, because \co{r3} appears in the ``exists'' clause and
...
not be, depending on one's definition.
A single-thread analysis would conclude that it is, because of the
...
This issue is not restricted to booleans.
For example, if given the code fragment shown in
Figure~\ref{lst:OOTA Cycle}
on
page~\pageref{lst:OOTA Cycle},
is the compiler permitted to act as if the code was instead as shown in
Listing~\ref{lst:Unused Inventing Non-Boolean Atomic Loads}?
Here, if the value loaded by line~9 is something other than 42,
the load is retried on line~11, and perhaps a third time on line~14.
If the value loaded in all three cases is something other than 42,
then line~16 stores the last value loaded, and otherwise, the constant
42 is stored by either line~18, 20, or~23.
Despite the fact that the code in both listings can be argued to be
equivalent, it is difficult to argue that a reasonable compiler
would act as if the code in
Figure~\ref{lst:OOTA Cycle}
was instead that shown in
Listing~\ref{lst:Unused Inventing Non-Boolean Atomic Loads}.
...
This section departs from one of the rules for this paper by considering
an example having undefined behavior.
