...
which also discusses other examples in which invented atomic loads lead
to incorrect behavior, that is, behavior that is not exhibited by the
...
But if the compiler is permitted to invent atomic loads, then this
guarantee might not hold, as can be seen from this transformed version of
the above code:
\begin{quote}
\begin{verbatim}
int r0 = x;
...
second load on line~15 could be removed without resulting in incorrect
behavior.
