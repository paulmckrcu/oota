...
\co{y}, \co{r1}, and \co{r2} have final values of 42, despite the fact
that there is nothing in the initial values or the executable code that
would support such an outcome:

\begin{enumerate}
\item   Line~6 loads from \co{x} into \co{r1}, somehow obtaining the
	value 42, never mind the \co{x}'s initial value of zero.
\item   Line~7 stores \co{r1}, and thus 42, to \co{y}.
\item   Line~12 loads 42 from \co{y} to \co{r2}.
\item   Line~13 stores 42 to \co{x}, justifying the value loaded by
...
dependency from a given load to a given store when a change in the value
loaded can change the value stored.}
...
above prohibition, and the term \emph{strict C++} to denote a hypothetical
standard that excludes this prohibition, but is otherwise identical
to full C++.
...
This paper will use the \co{herd7} tool to analyze fragments of C++ code.
This tool carries out the moral equivalent of full state-space searches
of concurrent code fragments, and further will in some cases identify
OOTA cycles.
...
Line~23 gives an example of a \co{locations} clause, which causes
\co{herd7} to dump out the final value of \co{x} and \co{y}.
...
on line~23, the values of \co{x} and \co{y} is also included, which
...
can be recast as similar \co{herd7} litmus tests, which areas shown in
...
Listing~\ref{lst:Simple OOTA}
that adds \co{z} along with lines~2 and~3.
...
threads no chance to modify \co{y}.
...
in \co{r2} that was loaded from \co{y} by line~2.
And given that the load from \co{z} no longer exists, it cannot possibly
...
\co{acyclic(sdep ; rfe)} in \co{herd7} notation), where rfe is the set
...
\subsection{Semantic Dependencies and Source Code}
...
value of \co{y} will not cause changes to the value of \co{x}.
...
\subsection{Semantic Dependencies Can Be Many To One}
\label{sec:Semantic Dependencies Can Be Many To One}

Suppose that both \co{y} and \co{z} are zero in the previous example.
Then changes to either but not both of \co{y} or \co{z} (again, all
else being equal) will not cause changes to the value of \co{x}.
In other words, there is no semantic dependency from either \co{y}
or \co{z} to \co{x} unless both \co{y} and \co{z} change.
...
\subsection{Semantic Dependencies Affected by Optimizations}
...
to \co{x}.
However, if the implementation knows that the value of \co{y} is
always the negative of \co{z} there is no semantic dependency and the
implementation can again act as if later accesses to \co{x} were instead
the constant zero, again, at least until the next assignment to \co{x}.
Please note that this leaves aside the question of how the compiler
would know this if \co{y} and \co{z} are atomic objects that cannot be
updated pairwise atomically and are subject to change at any time by
other threads.

\subsection{Semantic Dependencies Affected by \co{if} Statements}
\label{sec:Semantic Dependencies Affected by if Statements}

Consider the following simple \co{if} statement:
...
dependencies and using a variety of compilers should take the single-store
viewpoint~\cite{Howells2009membartxt},
the fact remains that these two viewpoints lead to differing conclusions
about whether this code represents one execution or two.
...
Not only that, we will take the notion of semantic dependence (and thus
OOTA) to be per-compiler rather than absolute.

For the special case of non-volatile atomic operations, we will further
restrict ourselves to compilers that do single-threaded analysis, with
the possible addition of specific constraints deduced from multi-threaded
analysis.
For volatile atomic operations, compilers are prohibited from inventing
loads, and we will restrict ourselves to compilers that extend that
prohibition to non-volatile atomic loads.
...
Which explains why some older hardware systems really do reorder prior
...
One possible explanation is ECC errors.
...
If the only possible reaction to an uncorrectable ECC error was to terminate
the program suffering that ECC error, there would be no problem.

However, some code has ways of handling even uncorrectable errors.
Operating-system kernels encountering an ECC error might note that the
affected data was being used by only one thread, and might react by
killing that thread.
...
Both the kernel and the user application would need to continue execution,
and might thus desire an exact exception.

Systems that offload processing of correctable ECC errors to software
also require exact exceptions.
After all, the software is going to need to be able to correct the
error and then fix up state as if the load had returned the fixed-up
value.

ECC errors are detected near the end of their corresponding load
instructions' execution, so that any need for exact exceptions rules
out the commiting of any subsequent stores before the load has fetched
its value.
...
\item	Some hardware might choose to forego ECC, for example, in order to
	reduce cost or to improve energy efficiency.
...
\item	Someone might come up with a clever way of correcting ECC errors
	...
	exceptions.
\item	As late as early 2024, some GPUs have been observed reordering
