...
and this is due to the fact that it does not model value-range compiler
optimizations.
...
based on value-range analysis (NOOTABOGA), for example, as shown
...
Of course this store could of course happen just before \co{P1()}'s
store to \co{y}, thus giving a superset of the outcomes for
...
or not this result is permitted makes this result even less friendly
to the developer than the ``mere'' RFUB in
Listing~\ref{lst:Read From Untaken Branch (RFUB)}.
This motivates excluding these outcomes, perhaps by requiring compilers
to treat non-omitted non-volatile atomic loads as though they were
volatile, at least with respect to the handling of the values that
they return.
A more direct and precisely targeted approach would be to forbid applying
