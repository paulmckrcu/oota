...
(We count read-modify-write instructions as comprising both memory loads and
memory stores.)
...
value zero.)
% Are we sure that no CPUs do this?
...
(the four coherence rules in 6.9.2.2p14--18 \co{[intro.races]}).
...
optimizations that combine multiple threads into one, but note
that such optimizations are more difficult than they first might
appear.
...
such reordering, despite a number requests to do
