...
It follows that these implementations hardly need to take any special
action to avoid OOTA.
In fact, for many compilers the constraints boil down to a simple
``Don't invent or duplicate atomic accesses'', which is probably
something the compiler wouldn't do anyway.
...
{\it Contra\/} that article, we will use the term \emph{loose C++}
...
Listings~\ref{lst:Simple OOTA}
and~\ref{lst:Simple Reordering}
...
and Listing~\ref{lst:Simple Reordering as Litmus Test}
on page~\pageref{lst:Simple Reordering as Litmus Test}, respectively.
...
ordering rather than a \emph{cause} of ordering, in that
...
in prior work.
In real-world C++ implementations, the standard's abstract machine
must be mapped onto real hardware, however, and this imposes additional
...
(the four coherence rules in 6.9.2.2p14--19 \co{[intro.races]}).
...
And duplicating an atomic load can even lead to incorrect results if done
carelessly.
...
\item[fre]
	From-read (external), which connects a load to any store to
...
\co{x} arrived at CPU~3 before arriving at CPU~0.
...
Therefore, the C++ compiler is within its rights to substitute
...
This preserved dependency helps prevent the formation of OOTA cycles
even in situations where there is no interference.\footnote{
...
Appendix~\ref{app:Non-LB Litmus Test},
...
an OOTA cycle (such as those indicated by their respective \co{exists} clauses)
...
each have only one OOTA cycle,\footnote{
	Note well that the initial values cannot be said to have come
	out of thin air.
	Although it is possible to have an OOTA cycle that does not
	involve the initial values, but happens to have the same value,
	the software would not be able to tell the difference.}
and an execution having initial values
of (say) two could have at most two OOTA cycles.
...
One might ask why semantic dependencies can be complicated when hardware
address, control, and data dependencies are much more straightforward.
The reason is that, unlike compilers, hardware:
(1)~Respects dependencies even when they reduce to a constant,
(2)~Optimizes much less aggressively, if at all, and
(3)~Lacks undefined behavior.
...
OOTA cycles nor OOTA-like behavior in full C++ compilers.
...
This means that a C++ compiler could prove that the value 17 is
always stored to \co{z} by examining \co{P0()} in isolation, given that
it is also legal for a C++ compiler to replace the value loaded
...
Given a compiler that proves that the value 17 is always
stored to \co{z}, there might be no semantic dependency to
\co{P0()}'s store to \co{z}, but must be to all of the other relaxed
atomic stores.
On the other hand, if the compiler does not prove the value
...
Either way, the value-17 OOTA cycle cannot occur in a compiler-based
C++ implementation.
Yes, a compiler that proves that the value 17 is always
...
\co{acyclic(sdep; rfe)}, as discussed in
Section~\ref{sec:OOTA: rf vs. rfe}.
...
If both dependencies are semantic dependencies, the OOTA cycle cannot
form.
...
For example, is a C++ compiler permitted to
duplicate \co{P0()}'s load from \co{x} and then make use of both loads'
values?
If this is permitted, then the compiler would be within its rights
to transform
...
If the compiler is further permitted to make a runtime choice
...
The compiler might also drop the load from \co{y}, whose value
...
C++ compilers should never duplicate nonvolatile
...
Is a C++ compiler within its rights to behave as if the code
shown in
Listing~\ref{lst:Only Atomic Increment}
...
\item	Any time a compiler might be tempted to invent uses from
...
In this case, compilers might hoist the load from x out of
the loop:
...
data races.
In theory, the compiler could make the above transformation without
...
This means that a compiler might be wise to adhere to the
...
enables the compiler to assume that the value of \co{r1} is
always zero.
This in turn allows the compiler to behave as if the expression
...
The compiler is then within its rights to act as if lines~13 and~14
were interchanged.
...
OOTA cycle on compiler-based C++ implementations.

\subsection{Multi-Cycle Litmus Test}
\label{app:Multi-Cycle Litmus Test}

Listing~\ref{lst:Multi-Cycle Litmus Test}
expands
Listing~\ref{lst:Causality Test Case 4}
in a different way, moving to four threads but having only the
original two variables \co{x} and \co{y}.
...
Therefore, none of these OOTA cycles can arise in a compiler-based
C++ implementation.
...
\subsection{Non-LB Litmus Test}
\label{app:Non-LB Litmus Test}
...
Therefore, an OOTA cycle cannot form in a compiler-based C++
implementation.

Note that in a C++ implementation that (erroneously) permitted an OOTA
cycle to form, that cycle would involve both of \co{P0()}'s \co{r0}
and \co{r1} local variables.
