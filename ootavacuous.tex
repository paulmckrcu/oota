...
% @@@ relaxed C++, OOTA C++, OOTA-agnostic C++, loose C++.
...
\item	A conditional branch instruction would have only one input:
	...
	A computed branch instruction would have as input the register
...
its source commits,
the action and outputs aren't fully determined until the inputs
are, and the instruction can't commit until its action and outputs are fully
determined.
...
outputs of instruction $I$ would lead to a change in the action or outputs of
...
fully determined whether the store and load will access the same address.
...
store instruction to generate an addressing exception.
...
This means there are stores $W_i$ in $A$, semantically depending on loads
...
Although we can are unable to prove it, we conjecture that in this
...
% @@@ Pull that section in here?  Drop this forward reference?
...
that can be used in \co{cbmc} test cases.
