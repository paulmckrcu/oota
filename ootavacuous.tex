...
until all of the computations that determine those effects remain
speculative.
...
Therefore, nonspeculative loads are prohibited from loading values from
speculative stores.
...
not be exposed to nonspeculative loads from other threads.
...
speculative stores cannot be visible to nonspeculative loads, lines~10
and~12 cannot be committed until after the load in line~8 completes
and the conditional on line~9 is evaluated nonspeculatively.
Similarly, lines~18 and~20 cannot be committed until after lines~16
and~17 complete nonspeculatively.
...
Nevertheless, the conditional store could not be committed while any
the instructions it depends on were still being speculatively executed.
...
While implementations are never supposed to break a semantic dependency,
