...
line~11 to \co{z}.
Only executions in which differ in the values loaded from both \co{x} and
\co{y} can cause the value stored to \co{z} to change to a non-zero value.
...
load from \co{y} returned non-zero, emphasizing the lack of any semantic
dependency between \co{x} and \co{z}.
...
and \co{y} on lines~9 and~10 are non-zero, executions that differ only
in the value loaded from either \co{x} or \co{y} will still affect the
