...
Then there is no semantic dependency from either to \co{x} unless both
\co{y} and \co{z} change.
...
dependency from the combination of \co{y} and \co{z} to \co{x}.
...
The two loads from \co{y} are fungible, and one should therefore expect
a compiler to replace this \co{if} statement with a single load from \co{y}.
...
``semantic dependency'' are problematic.
...
This would mean that the store to \co{z} was always the constant one,
...
However, we use the convention that the \co{exists} clause is observable
behavior.
...
\item	Implementations should not invent uses for values from
	...
	But are there any useful optimizations that invent such uses?
	(One can argue that in all such cases, the loads should instead
	be fused into a single load.)
\item	Listing~\ref{lst:Inventing Atomic Loads}
	looks like it has an OOTA cycle, but there is in fact no
	semantic dependency.
	But then how exactly does one identify a semantic dependency?
\item	What other equally interesting situations might come to light?
\item	Your reaction here!
\end{enumerate}
...
% Construct nondeterminism example, for example, based on i++.
% Check cmbc on nondeterministic i++.
% Use LKMM as check for dependencies?  Check invented-load litmus tests.
