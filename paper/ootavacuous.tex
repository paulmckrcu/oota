...
Paul E.~McKenney (paulmck@kernel.org) \and
...
stores~\cite{Boehm:2014:OGA:2618128.2618134,HansBoehm2019OOTArevisitedAgain,Lahav:2017:RSC:3062341.3062352,PiezhaoOu2018OOTApapl},
...
None applied real-world hardware ordering constraints (based on
long-standing laws of physics) to the problem of avoiding OOTA cycles.

Augmenting C++ abstractions with these constraints would of course
limit applicability of this paper's results to C++ programs running
on real-world computer systems.
However, a quarter century of OOTA research based on pure C++ abstractions
has resulted in incremental progress at best, and real-world computer
systems comprise an exceedingly important part of the overall OOTA
problem.
...
Therefore, this paper's contribution is to show how these constraints
prevent real-world C++ implementations from exhibiting OOTA behavior,
and not with minimal cost~\cite{PiezhaoOu2018OOTApapl}, but rather with
no cost at all.
...
Although it is unexpected, and while we don't recommend this sort of code
transformation, we cannot say that this conclusion is definitely wrong,
because our intuitive notions of semantic dependency are not clear in
cases where multiple loads of the same variable are present.
% Can we instead say that transforming the code might require that the
% witness also be transformed?
In the unlikely event that this transformation proves useful, there will be
a community process that determines whether this transformation is valid,
that is, whether value of the use cases is worth the added conceptual load
on developers.
Until that time, it is impossible to say whether or not there is
a semantic dependency from \co{y} to \co{z} without reference to a
...
(2)~Consider various classes of global analysis, such as
demonstrating that a given expression will always evaluate to a constant;
(3)~Examine the possibility of ``flattening'' optimizations that combine
multiple threads into one;
(4)~Explore cases in which semantic dependencies must be absolute,
not relative to any particular compiler;
(5)~Include the effects of input and output or other operations which
can vary from one execution to another; and
(6)~Delineate more precisely the limits of permissible behavior for
