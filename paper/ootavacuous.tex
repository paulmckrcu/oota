...
\begin{abstract}
	...
	atomics.
	For implementations doing only per-thread analysis and
	optimization, we accommodate nonvolatile atomics by defining
	``quasi-volatile'' behavior, adherence to which avoids situations
	in which arithmetic-expression evaluation involving atomic
	accesses could otherwise violate the standard.
...
code~\cite{PaulEMcKenney2020RelaxedGuideRelaxed}.\footnote{
	...
	followed by a number denote CPU registers, and other names denote
	shared variables.
	All shared-variable accesses are relaxed atomic.}
First, P0's line~1 does a load from \co{x} into \co{r1},
...
Either the compiler or the CPU might reorder P1's lines~1 and~2, so that
...
However, this OOTA result has not been observed on real implementations
due to the default-zero initialization combined with semantic dependencies
