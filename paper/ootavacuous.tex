...
\begin{abstract}
	...
	will adhere to if they confine themselves to single-thread analysis.
\end{abstract}
\maketitle{}
...
In fact, for many compilers the constraints boil down to a simple ``Don't
invent or duplicate atomic accesses'', which the compiler should not
do anyway if it is to evaluate arithmetic expressions involving atomic
accesses in accordance with the abstract machine.
...
Throughout this paper, variables whose names are ``\co{r}'' followed by
a number denote registers, and other names denote shared variables.
...
behavior caused by simple reordering, such as that shown in
...
This paper follows P2055R0~\cite{PaulEMcKenney2020RelaxedGuideRelaxed}
in using \emph{full C++} to denote the standard including the
prohibition of OOTA mentioned above.
Unlike that article, we use \emph{loose C++}
...
Many of these workers appear to have abandoned this effort, but as of
2024, Mark Batty is persisting with modular relaxed dependencies, which
may have the added ability of checking compiler optimizations.
@@@ cite web site.
...
% concerns.
This paper focuses on examples lacking UB.
...
\emph{Can we summarize this section, especially given that the ASPLOS
audience can be expected to have a greater understanding of these issues
than the C++ standards committee?}
...
\emph{Can we summarize this section with a list of permitted and prohibited
optimizations in the discussion of quasi-volatile?}
...
Without P0's line~1, only the values~0 and~3 can be loaded into \co{r1} by
P1's line~1.
With P0's line~1, the additional value~42 can also be loaded into \co{r1}.
...
Without P0's line~1, if the value loaded into \co{r1} is one then the final value
of \co{x} must be two.
With that line, the final value of \co{x} can be one even when the
...
In contrast, compilers should not duplicate, repurpose, or otherwise
invent atomic loads, as discussed in
...
Section~\ref{sec:Issues and Refinements}
