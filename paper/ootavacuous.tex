...
\begin{quote}
	Given:
	(1) Conventional hardware that does not expose speculative stores
	to other threads,
	(2) A compiler that restricts itself to thread-at-a-time analysis
	and treats non-volatile atomic accesses as if they were volatile
	(with the exceptions noted below), and
	(3) A program that is free of undefined behavior,
	this program is guaranteed not to compute out-of-thin-air values.
	The guarantee is also provided by implementations that optimize
	non-volatile atomics (when permitted by the as-if rule) by
	omitting accesses, merging accesses to the same object, and/or
	reordering accesses to different objects.
	Implementations that optimize more aggressively are still
	encouraged, but their out-of-thin-air properties should be
