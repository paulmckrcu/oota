...
\begin{abstract}
	...
	implementers.
	However, we know of no instances of OOTA behavior in real C++
	implementations.
	...
	thus permitting easier analysis of semantic dependencies and
	OOTA behaviors, thanks to existing formal hardware models.
	We show that these models' constraints prevent OOTA cycles from
	occurring in undefined-behavior-free C++ programs running on
	compiler-based implementations, provided the cycles involve only
	volatile atomics.
	We also extend this work to nonvolatile atomics by defining
	``quasi-volatile'' behavior that we C++ implementations
	must adhere to in order to evaluate arithmetic expresssions
	involving atomic accesses in accordance with the abstract
	machine.

	It follows that such implementations need take no special action
	to avoid OOTA, and that no changes are required to user code or
	to the standard.
\end{abstract}
\maketitle{}
...
The need to prohibit simple OOTA is one reason why compiler-based value
speculation optimizations require checks on such speculation, and further
requires that these checks must be based on the values actually loaded.
