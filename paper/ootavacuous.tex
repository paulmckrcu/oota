...
\begin{abstract}
	...
	thus easing analysis of semantic dependencies and OOTA behaviors
	via existing hardware models.
	We show that these models' constraints prevent OOTA cycles from
	occurring in undefined-behavior-free C++ programs running on
	compiler-based implementations that use single-thread analysis,
	provided the cycles involve only volatile atomics.
	We also extend this work to nonvolatile atomics by defining
	``quasi-volatile'' behavior that C++ implementations must adhere
	to in order to evaluate arithmetic expressions involving atomic
	accesses in accordance with the standard.
...
may have the added ability of checking compiler
