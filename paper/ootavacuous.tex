...
recommendation:
\begin{quote}
	% Consider moving the parenthesized statement to its own sentence.
	Compiler-based implementations whose binaries run on conventional
	hardware (for example, hardware that does not expose speculated
	values to other CPUs or threads) are guaranteed not to compute
	out-of-thin-air values in programs that are free of undefined
	behavior, as long as they restrict themselves to thread-at-a-time
	analysis and treat non-volatile atomic accesses as if they
	were volatile.
	Except that, when permitted by the as-if rule, they may omit
	accesses, merge accesses to the same object, or reorder accesses
	to different objects.
	Optimizations that do not meet these criteria are still
	encouraged, but their out-of-thin-air properties must be carefully
