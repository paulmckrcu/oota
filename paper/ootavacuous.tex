% \documentclass[10]{article,prologue,table,svgnames}
...
\newcommand{\hlg}[1]{\sethlcolor{lightgreen}\hl{#1}\sethlcolor{yellow}}
\newcommand{\hlr}[1]{\sethlcolor{red}\hl{#1}\sethlcolor{yellow}}
\definecolor{lightblue}{rgb}{0.7, 0.7, 1.0}
\newcommand{\hlb}[1]{\sethlcolor{lightblue}\hl{#1}\sethlcolor{yellow}}

\begin{document}
\title{P3692R4: How to Avoid OOTA Without Really Trying}
...
\begin{abstract}
	...
	behavior and show that for existing implementations doing per-thread
	analysis and optimization, this behavior not only avoids OOTA
	but also is necessary to avoid
	...
	action from implementations and no changes to user code.
	As a consequence, we are asking for only for a small non-normative
...
\begin{itemize}
\item	P3692R4 updates the C++ working draft to N5032 and applies
	feedback from CWG.  It also adds a citation to the NVIDIA PTX
	prohibition against OOTA.
\item	P3692R3 fixes an ``and and'' typo, updates the proposed note
	to be added to the standard, and adds authors..
\item	P3692R2 applies feedback from the 2025 Sophia meeting,
	including reworking the requested change to the standard
	in terms of volatile semantics (courtesy of Olivier Giroux),
	states this paper's benefits to other OOTA researchers, and
	updates the author list.
\item	P3692R1 adds contact information, defines ``preserved
	dependencies'' that are a subset of syntactic and a superset of
	semantic dependencies, and makes numerous changes in the name
	of wordsmithing and clarification.
\item	This paper is a condensation of P3064R2 (``How to Avoid OOTA
	Without Really
	Trying'')~\cite{PaulEMcKenney2023P3046R2/OOTAwithoutTrying},
	which was criticized as being overly long.
	Voluminous off-list discussions are reflected.
...
code~\cite{PaulEMcKenney2020RelaxedGuideRelaxed}.\footnote{
	Throughout this paper, we use names beginning with ``\co{r}''
...
Because nothing in the C++ memory model rules out such OOTA cycles
(as indicated by the ``sometimes satisfied'' in the figure), the C++
standard explicitly recommends against them in 32.5.4p8
(\co{[atomics.order]})~\cite{ThomasKoeppe2025N5032}:
\begin{quote}
...
of ``carries a dependency'' in 6.9.2.2p7 (\co{[Data races]}).
...
stores~\cite{Boehm:2014:OGA:2618128.2618134,HansBoehm2019OOTArevisitedAgain,Lahav:2017:RSC:3062341.3062352,PiezhaoOu2018OOTApapl,SungHwanLee2023WeakMemoryPromising,LukeGeeson2023TightenRelaxed},
but this can require executing real
instructions~\cite[Section 7.1]{Maranget2012TutorialARMPower}~\cite{RichardGrisenthwaite2023TightenRelaxedJustSayNo}~\cite[Section 8.10.4]{NVIDIA-PTX2026},
...
ARMv7, PowerPC, or NVIDIA GPGPUs.
...
These simple-seeming notions conceal considerable complexity,
...
%and not with minimal cost~\cite{PiezhaoOu2018OOTApapl},
in many cases with no cost at
all~\cite{PaulEMcKenney2023P3046R2/OOTAwithoutTrying}.
In addition, our results simplify the job of OOTA researchers seeking
practical impact by allowing them to ignore C++ implementers' need for
extreme simplicity and performance.
This in turn permits such researchers to focus on advanced topics such
as formal-verification tooling and evaluation of optimizations.
...
as long as \co{y} is zero, changes in the
value of \co{x} will not cause a change in the value stored to \co{z}.
As a result, the semantic dependency from \co{x} to \co{z} exists only
in executions where \co{y} is nonzero,
...
both \co{x} and \co{y} are zero.
Then a change to exactly one of \co{x} or \co{y}
will not cause a change in the value stored to \co{z}.
In other words, in this execution there is no semantic dependency
from either \co{x} or \co{y} to \co{z}.
But there \emph{is} a semantic dependency from the \emph{pair}
\{\co{x},~\co{y}\} to \co{z},
because changes to both \co{x} and \co{y}
can cause the value stored in \co{z} to change.
...
semantic dependency from \co{x} to \co{y} without reference to the
definition of \co{f()}.
In addition, \co{f} might well be a pointer to a function, in which case
the existence of a semantic dependency from \co{x} to \co{y} might well
vary at runtime as the value of function pointer \co{f} changes.

\subsubsection{Semantic Dependencies and Free Choices}
\label{sec:Semantic Dependencies and Free Choices}

Consider the following example, with variable \co{i} initially zero:
% @@@ Citation?
% @@@ "Outlawing ghosts: Avoiding out- of-thin-air results" has a
% @@@    ++ example, but is concerned about hoisting that before
% @@@    an "if" statement.
\begin{quote}
\begin{verbatim}
int foo(int a, int b)
...
which \co{char} is architecturally a signed type,
...
This implies that volatile load-acquires might not synchronize with
...
so they might not contribute to the happens-before relation.
Also, in these environments the rfe relation does not necessarily apply
to volatile loads and stores, and hence results that appear to indicate
an OOTA cycle might instead be due to the nonbenign environment causing
loads to return unpredictable values.
Thus the accesses in a {\it bona-fide} OOTA cycle executing in such an
environment must be nonvolatile.
...
to replace the expression with a constant 0.
...
was zero and the load into \co{rinvented} happened when the value of
\co{x} was 10, then the value of \co{r1} would be 21, which is not
a perfect square.
...
\end{verbatim}
\end{quote}
It seems clear that the store to \co{z} semantically depends on the load
...
\end{verbatim}
\end{quote}
Here \co{r1a} represents the original load from \co{x} while the \co{r1b}
...
\item	Check for an assembly level syntactic dependency between
	...
	\item	Otherwise, there might be a semantic
...
this paper.
% Should we call out the relationship to the perturbation method of P0422r0?
...
\end{verbatim}
\end{quote}
If the compiler knows nothing about the function \co{f()} and the variable
\co{x}, it must emit code that loads from \co{x}, passes this value to
\co{f()}, and finally stores the return value to \co{y}.
Is there a semantic dependency from the load of \co{x} to the store of
\co{y}?
To determine this, it is necessary to apply this same analysis to the
definition of \co{f()}, treating its argument as a load and each of its
\co{return} statements as a store.
If and only if this analysis shows that, for a given execution, there
is an assembly level syntactic dependency within \co{f()}, then there
must also be a semantic dependency from the load of \co{x} to the store
of \co{y}.
As always, if the compiler cannot prove that there is no such semantic
dependency, it must assume than there is one.

If \co{f} is a pointer to a function, then the compiler must also emit
a load from \co{f}, so that there might also be a semantic dependency
from the load from \co{f} to the store to \co{y}.
In the context of C++, this is straightforward:
A given execution will load a pointer to a particular function from
\co{f}, so if there is another execution that loads a pointer to a
different function, and the two functions return different values for
the value loaded from \co{x}, then there is a semantic dependency from
\co{f} to \co{y}.
Languages that permit functions to be generated at runtime must use
more complicated analysis.

Situations where the compiler knows about \co{f} often result in
...
\end{verbatim}
\end{quote}
This has potential semantic dependencies between the loads from \co{x}
and \co{y} to the store to \co{z}.
The corresponding assembly code can vary depending on what the compiler
knows about the values of \co{x} and \co{y}.

If it knows nothing, then it must load from both, multiply the values
loaded, and then store the resulting product to \co{z}.
...
(resulting in the value 1 being stored to \co{z}),
then the execution in which the value 2 is loaded from \co{x} witnesses
a semantic dependency from \co{x} to \co{z}.
Similarly, the execution in which the value 2 is loaded from \co{y}
witnesses a semantic dependency from \co{y} to \co{z}.

But suppose we instead start with the execution where the value 0
is loaded from \co{x} and the value 1 is loaded from \co{y}.
In this case, there is no witness execution that varies only the value
of \co{y}, as all such executions will result in the value 0 being stored
to \co{z}.
However, the execution where the value -1 is loaded from \co{x}
witnesses a semantic dependency from \co{x} to \co{z}, resulting in
the value -1 being stored to \co{z}.

But suppose we instead start with the execution where the value 0
is loaded from both \co{x} and \co{y}.
There is again no witness execution that varies only the value of \co{y},
but there is also no witness execution that varies only the value of \co{x},
all such executions still resulting in the value 0 being stored to \co{z}.
However, the execution where the value 2 is loaded from \co{x} and the
value 3 is loaded from \co{y} witnesses a semantic dependency from the
set \{\co{x}, \co{y}\} to \co{z}, resulting in the value 6 being stored
to \co{z}.

Alternatively, if the compiler knows that the value loaded from \co{x}
is guaranteed to be zero, then it can omit the loads from both \co{x}
and \co{y} (assuming that these values are not otherwise used), and
then simply store the constant 0 to \co{z}.
But to know that the value of \co{x} will always be 0, the compiler
must perform a global analysis.
Our requirement that such compilers treat all atomic accesses as volatile
would then forbid the compiler from omitting the loads from \co{x}
and \co{y}, thereby forcing the dependency to be preserved.
...
This fact is intuitively obvious, but let us describe in detail
...
On the other hand, forcing global-analysis compilers to treat all atomic
accesses as though they were volatile could prevent other behaviors
which, although they are not OOTA, are nevertheless equally unexpected
and confounding to
programmers~\cite[Appendix D.2]{PaulEMcKenney2023P3046R2/OOTAwithoutTrying}.
% @@@ Update to P3064R3 when available.
While such considerations are worth pursuing further, they are not within
...
Although many optimizing compilers would omit the loads into \co{r1}
and \co{r3}, or merge them with \co{r2} if they are used later, one
...
Although this result is unexpected, and while we don't recommend this sort of
...
\co{y} to \co{z} without reference to a specific implementation.\footnote{
	At the 2025 C++ Standards Committee meeting in Sofia, it was
	pointed out that the added ``\co{if (r1 != r3)}'' can be argued
...
\subsection{Wording Change to the Standard}
\label{sec:Wording Change to the Standard}

As noted in Section~\ref{sec:Brief OOTA Overview}, the C++
standard recommends against OOTA cycles in 32.5.4p8
(\co{[atomics.order]})~\cite{ThomasKoeppe2025N5032}:
\begin{quote}
	Implementations should ensure that no ``out-of-thin-air'' values
	are computed that circularly depend on their own computation.
...
recommendation?
We therefore advocate adding the two non-normative notes shown in
Listing~\ref{lst:Wording Additions to the Standard After 32.5.4p9 ([atomics.order])}
to this recommendation after 32.5.4p9 (\co{[atomics.order]}
(which begins with ``[Note 6 : The recommendation similarly disallows
r1 == r2 == 42 in the following example'').

\begin{listing*}[tbp]
\begin{quote}
	[Note 6 : The recommendation similarly disallows r1 == r2 ==
	42 in the following example...

	~

	\hlg{[ Note 7 --- An implementation that translates to instruction
	sequences for a physical machine and treats non-volatile
	atomic accesses as though they were volatile does not produce
	out-of-thin-air values for a program free of undefined behavior.
	An implementation that performs only thread-at-a-time analysis
	in support of optimizations that omit and merge non-volatile
	atomic accesses to the same object and reorder non-volatile atomic
	accesses to different objects (when permitted by the as-if rule)
	and otherwise treats non-volatile atomic accesses as though
	they were volatile does not produce out-of-thin-air values for
	a program free of undefined behavior.
	The possibility of out-of-thin-air values remains for
	implementations not meeting these restrictions.  --- end note.]}

	~

	\hlg{[ Note 8 --- Unlike abstract machines, physical machines
	are subject to instruction-execution and information-transfer
	latencies.
	Physical machines carefully manage any speculative execution so as
	to avoid generating out-of-thin-air values at the hardware level.
	--- end note. ]}
\end{quote}
\caption{Wording Additions to the Standard After 32.5.4p9 (\co{[atomics.order]})}
\label{lst:Wording Additions to the Standard After 32.5.4p9 ([atomics.order])}
\end{listing*}
...
implementations.
% This work also demonstrates the need for caution when optimizing
% atomic operations.
