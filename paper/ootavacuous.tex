...
\begin{document}
\title{P3692R3: How to Avoid OOTA Without Really Trying}
...
instructions~\cite[Section 7.1]{Maranget2012TutorialARMPower,RichardGrisenthwaite2023TightenRelaxedJustSayNo},
...
volatile store-releases in the sense of the C++ memory model,\footnote{
	For example, they might instead synchronize with store-releases
	in device firmware (or vice versa), roughly speaking.}
...
to volatile loads and stores, and hence the concept of an OOTA cycle
applies only to benign environments.
...
\begin{listing}[tbp]
@@ DisplayRunLitmus litmus/oota-data+lt100.litmus @@
\caption{OOTA and Value Range Analysis}
\label{lst:OOTA and Value Range Analysis}
\end{listing}

A less malicious example is shown in
Listing~\ref{lst:OOTA and Value Range Analysis},\footnote{
	Kudos to Hans Boehm for providing this example.}
in which both \co{x} and \co{y} have initial values of zero.
Global analysis would show that there are no stores to either \co{x}
or \co{y} having values greater than 42.
The compiler could then omit T1's \co{if} statement, thus unconditionally
storing the value \co{42} to \co{x}.
This would allow both the compiler and the CPU to reorder T1's load
and store, which could in turn produce the OOTA-like result of
\co{x=y=42}.

This unintuitive behavior could not occur if the two loads were not
omitted in the transformed program derived from
Listing~\ref{lst:Simple OOTA},
nor if the compiler was forbidden from assuming that the values returned
by a load from a variable were restricted to the set of values stored
to that same variable in
Listing~\ref{lst:OOTA and Value Range Analysis}.
...
quasi volatile except that: (1)~It was not allowed to omit accesses to them
and (2)~It was required to assume a non-benign environment.
% @@@ Does (2) suffice?  After all, in a non-benign environment, the
% @@@ compiler could not assume that the simple OOTA case would in fact
% @@@ produce a given OOTA result, right?
...
recommendation?
We therefore advocate adding the following two non-normative notes to this
recommendation, but following the notes giving examples of OOTA cycles:
\begin{quote}
	[ Note N --- A compiler-based implementation that runs on
	real-world hardware and treats non-volatile atomic acesses as
	though they were volatile will not produce out-of-thin-air values
	for a program free of undefined behavior.
	An implementation that performs only thread-at-a-time
	analysis/optimization and that omits and merges non-volatile
	atomic accesses to the same object and reorders non-volatile
	atomic accesses to different objects (when permitted by the as-if
	rule), but that otherwise treats non-volatile atomic accesses
	as though they were volatile, will not produce out-of-thin-air
	values for a program free of undefined behavior.
	Implementations not meeting these restrictions require extra
	care to ensure that they do not compute out-of-thin-air values.
	--- End note.]

	[ Note N+1 --- In this context, real-world hardware is
	subject to physical limitations on instruction-execution and
	information-transfer latencies, as opposed to abstract platforms
	with no such limits.
	Real-world hardware must also carefully manage any speculative
	execution so as to avoid generating out-of-thin-air values at
	the hardware level.
	--- End note. ]
\end{quote}
