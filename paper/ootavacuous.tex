...
\begin{abstract}
	...
	atomics.
	For implementations doing only single-threaded analysis, we
	accommodate nonvolatile atomics by defining ``quasi-volatile''
	behavior that must be adhered to in order to evaluate arithmetic
	expressions involving atomic accesses in accordance with the
	standard.
...
values of 42, despite their being no instances of 42 in the default-zero
initialization or in the executable
code~\cite{PaulEMcKenney2020RelaxedGuideRelaxed}.\footnote{
	Throughout this paper, variables whose names are ``\co{r}''
	followed by a number denote registers, and other names denote
	shared variables.
	All accesses to shared variable are relaxed atomic accesses.}
...
Finally, P1's line~2 stores 42 to \co{x}, justifying the value loaded
by P0's line~1.
...
which prevents misapplication of the as-if rule in oracular C++
implementations:
\begin{quote}
...
until they have determined its value,\footnote{
	...
	This same restriction applies to compiler-based speculation.}
...
Both the compiler and the CPU might reorder P1's lines~1 and~2, so that
all of \co{x}, \co{y}, \co{r1}, and \co{r2} might have the value 42,
...
On the other hand, OOTA cycles can arise even when value 42 is mentioned,
as shown in
Listing~\ref{lst:OOTA Cycle}.
Line~3 of both processes stores the value 42 only when line~2 determines
that the value loaded was 42.
Default-zero initialization prevents this in both full C++ and (as we will
see) real-world loose C++, but not an oracular loose C++ implementation.

The next section looks at prior work on these issues.
...
This paper's contribution is showing how these constraints prevent
...
\emph{Tagline: ``Semantic dependencies are more complicated than you think.''}

This section formulates OOTA cycles as cycles in sdep $\cup$ rfe instead
of the traditional sdep $\cup$ rf, then shows that sdep is not a strict
...
% \end{quote}
This is an elaboration of P1 from
Listing~\ref{lst:Simple OOTA}
...
\co{r2}.
This means that line~4 stores the same value to \co{z} that was stored
to \co{y} by line~2.
...
Later, we will see examples of this destruction.
...
\emph{Tagline: ``Need to look at machine code.''}

This section looks more carefully at executions from the viewpoint of
...
(3)~(There is a condition on the timing and interleaving of input and output,
...
then uses this information to define necessary properties of
\emph{quasi-volatile} atomic operations.
...
in which P0 loads the value of \co{x} and stores it in \co{y} while P1
does the reverse.
...
a perfect square.
This fails to provide the abstract machine's guarantees.
...
Applying these requirements to specific examples results in prohibiting
invention or duplication of atomic loads and stores.
However, in the absence of ordering constraints, it permits omitting
...
This section examines dependencies at the machine-instruction level,
then combines this with mapping from the previous section to define the
...
Any hardware dependency ultimately emanates from register initial values,
load instructions, immediate values, or I/O accesses.
We set aside I/O, so that load instructions are the only sources
...
A CPU is clearly not permitted to commit an instruction its inputs have
been committed, unless the remaining uncommitted inputs cannot affect
that instruction's outputs.
For instance, a conditional-move instruction needn't wait for the source
input to commit if it knows that the condition is definitely false.
...
CPUs, owing to finite speed of light and non-zero size of computing
hardware.
...
To see this, suppose that abstract execution $A$ is realized by hardware
execution $H$ and $A$ does have an OOTA cycle.
...
more tractable.
This insight allows us to demonstrate that OOTA cycles cannot form
...
dependencies in P0 and P1 would not be semantic.
...
local per-thread anaylsis and optimization.
