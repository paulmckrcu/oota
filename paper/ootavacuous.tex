...
\begin{document}
\title{DNNNNR0: How to Avoid OOTA Without Really Trying}
...
\begin{abstract}
	...
	behavior which enables existing implementations that do per-thread
	analysis and optimization not only to avoid OOTA, but also to
	avoid incorrectly evaluating arithmetic expressions involving
	atomic accesses.
...
which was criticized as being too long.
...
definition of \co{f()}.
For some definitions of \co{f()}, a semantic dependency exists, and for
others it does not.

Note that \co{f} might well be a pointer to a function, in which case
the existence of that semantic dependency might well vary at runtime as
the value of function pointer \co{f} changes.
...
This section exercises the definition of ``semantic dependency''
presented in the previous section on selected examples.
...
\co{y}, it must emit code that loads from \co{x}, passes this value to
...
is a semantic dependency within \co{f()}, then there must also be a
semantic dependency from the load of \co{x} to the store of \co{y}.
Of course, if the compiler cannot prove that there is no such semantic
dependency, it must assume than there is one.

If \co{f} is a pointer to a function, then the compiler must also
generate load from \co{f}, so that there might be a semantic dependency
from either or both of \co{f} and \co{y} to the store to \co{y}.
...
that execution's value from \co{y}, then there is a semantic dependency
from \co{f} to \co{y}.

Situations where the compiler knows about \co{f} more often result in
...
Both witnesses result in the value 2 being stored to \co{z}.
...
witnesses a semantic dependency from \co{x} to \co{y}, resulting in
the value -1 being stored to \co{z}.
Furthermore, the execution where the value -1 is loaded from \co{x}
and the value -2 is loaded from \co{y} witnesses a semantic dependency
from the set \{\co{x}, \co{y}\} to \co{z}, resulting in the value 2 being
stored to \co{z}.
...
all such executions resulting in the value 0 stil being stored to \co{z}.
However, the execution where the value 2 is loaded from both \co{x} and
\co{y} witnesses a semantic dependency from the set \{\co{x}, \co{y}\}
to \co{z}, resulting in the value 4 being stored to \co{z}.
...
and \co{y} (assuming that the value from \co{y} is not otherwise used),
storing the constant 0 to \co{z}.
There can be no semantic dependencies from these now-nonexistent loads
to the store to \co{z}.
And then any execution that witnessed a semantic dependency from
either or both of \co{x} and \co{y} would indicate a bug in the
compiler, which would have been wrong in its assumption of the
guarantee that \co{x} was zero.
